...
public class FcCommand implements IClientCommandListener
{
	...
	{
		mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX+"_invalid_operation"));
		return true;
	}
	...
	private boolean executeAdd(List<String> args, Minecraft mc)
	{
		if (mc.level != null && mc.getCameraEntity() != null)
		{
			...
			{
				mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX+"_already_in_use"));
			}
		}
	...
	private boolean executeSet(List<String> args, Minecraft mc)
	{
		if (mc.level != null && mc.getCameraEntity() != null)
		{
			...
			try
			{
				...
				{
					mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX+"_not_found", String.format("%02d", id)));
				}
			}
			catch (Exception err)
			{
				Tweakeroo.LOGGER.error("FcCommand#set(): Exception; {}", err.getLocalizedMessage());
				mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX+"_invalid", args.getFirst()));
			}
		}
	...
	private boolean executeList(List<String> args, Minecraft mc)
	{
		if (mc.level != null)
		{
			...
			if (list.isEmpty())
			{
				mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX+"_list_empty"));
			}
			else
			{
				for (CameraPreset entry : list)
				{
					mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX+"_list", entry.toShortString()));
				}
			}
		}
	...
	private boolean executeRename(List<String> args, Minecraft mc)
	{
		...
		try
		{
			...
			if (CameraPresetManager.getInstance().hasId(id))
			{
				...
				{
					mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX + "_not_enough_args_given"));
				}
			}
			else
			{
			...
			if (CameraPresetManager.getInstance().hasId(id))
			{
				...
				if (preset != null && mc.level != null)
				{
					if (mc.level.dimension().identifier().equals(preset.getDim()))
					{
						...
						{
							mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX + "_matches_camera", String.format("%02d", preset.getId())));
						}
					}
					else
					{
						mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX + "_wrong_dimension", String.format("%02d", preset.getId()), preset.getName()));
					}
				}
			}
			else
...
//		mc.inGameHud.getChatHud().addClientSystemMessage(StringUtils.translateAsText(PREFIX+"_not_implemented", args.getFirst()));
		...
		if (mc.level != null)
		{
			...
			{
				mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(PREFIX+"_cycle_not_found"));
			}
		}
	...
	private boolean executeHelp(List<String> args, Minecraft mc)
	{
		...
		if (!args.isEmpty())
		{
			...
			if (sub != null)
			{
				...
				mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(prefix));
				mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(prefix+"."+key));

				if (!sub.getAlias().isEmpty())
				{
					mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(prefix+"_alias", sub.getAlias().toString()));
				}
			}
			else
			{
		...
		else
		{
			...
			for (Sub entry : Sub.values())
			{
				...
				{
					mc.gui.hud.getChat().addClientSystemMessage(StringUtils.translateAsText(prefix+"_alias", entry.getAlias().toString()));
				}
			}
		}
